...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            ...
            score1 = score1 + take_turn(num2, score0, dice)
    ...
    def say(score0, score1, last_score=last_score, running_high=running_high):
        if who == 1:
            if score1 - last_score > running_high:
                print(abs(score1 - last_score), 'point(s)!',
                      'The most yet for Player', who)
                running_high = score1 - last_score
            return announce_highest(who, score1, running_high)
        else:
            if score0 - last_score > running_high:
                print(abs(score0 - last_score), 'point(s)!',
                      'The most yet for Player', who)
                running_high = score0 - last_score
            return announce_highest(who, score0, running_high)
    return say

    #     if abs(run - running_high) > running_high:
    #         print(abs(run - running_high), 'point(s)!',
    #               'The most yet for Player', who)
    #     return announce_highest(who, last, abs(run - running_high))
    # return say
...
def make_averaged(original_function, trials_count=1000):
    ...
    def avg(*args,trials_count = trials_count):
        sum = 0
        count = trials_count   #这里tiral_count值不知道
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while num_of_dice > 0:
        #print(roll_dice(num_of_dice,dice))
        cur = make_averaged(roll_dice,trials_count)(num_of_dice,dice)
        #print(cur)
        if cur >= max_roll:
            max_roll = cur
    ...
    turn = 0
    while i<= trials_count:
        if  make_averaged(dice,trials_count)() > high:
            high =  make_averaged(dice,trials_count)()
            turn = i
    return turn
