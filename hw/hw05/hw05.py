class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            if self.balence < self.price:
                return 'You must add ${0} more funds.'.format(self.price-self.balence)
            elif self.balence == self.price:
                self.balence =0
                self.stock -= 1
                return 'Here is your {0}.'.format(self.good)
            else:
                word = 'Here is your {1} and ${0} change.'.format((self.balence - self.price),self.good)
                self.balence = 0
                self.stock -=1 
                return word
    ...
    def add_funds(self,money):
        if self.stock == 0:
            ...
            self.balence += money
            return 'Current balance: ${0}'.format(self.balence)
    def restock(self,stock):
        self.stock += stock
        return 'Current {1} stock: {0}'.format(self.stock,self.good)
    ...
    if n < 10:
        ...
        for i in range(1,10):
            if n // pow(10,i) == 0:
                tmp = pow(10,i-1)
                break
        return Link(n//tmp,store_digits(n%tmp))
